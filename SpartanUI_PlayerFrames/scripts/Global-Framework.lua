...
-- local base_plate4 = [[Interface\AddOns\SpartanUI_PlayerFrames\media\base_plate4]]
...
local CreatePlayerFrame = function(self,unit)
	...
	do -- setup status bars
		do -- cast bar
			...
			cast:SetFrameStrata("LOW"); cast:SetFrameLevel(2);
		...
		do -- health bar
			...
			health:SetFrameStrata("LOW"); health:SetFrameLevel(2);
		...
		do -- power bar
			...
			power:SetFrameStrata("LOW"); power:SetFrameLevel(2);
...
local CreateTargetFrame = function(self,unit)
	...
	do -- setup buffs and debuffs
		-- Buff's are Tainted, need to figure out how to use SecureAuraHeaderTemplate found in SecureGroupHeaders(.xml & .lua)
		self.Auras = CreateFrame("Frame",nil,self);
		self.Auras:SetWidth(22*10); self.Auras:SetHeight(22*2);
...
local CreateTargetFocusFrame = function(self,unit)
	self:SetWidth(210); self:SetHeight(60);
	do -- setup base artwork
		local artwork = CreateFrame("Frame",nil,self);
		artwork:SetFrameStrata("LOW");
...
local CreateUnitFrame = function(self,unit)
	...
	self:SetFrameStrata("LOW"); self:SetFrameLevel(1);
	self:SetScript("OnEnter", UnitFrame_OnEnter);
	self:SetScript("OnLeave", UnitFrame_OnLeave);
	self:RegisterForClicks("anyup");
	self:SetAttribute("*type2", "menu");
	self.colors = addon.colors;
	return (unit == "target" and CreateTargetFrame(self,unit)) or (unit == "targettarget" and CreateToTFrame(self,unit)) or (unit == "player" and CreatePlayerFrame(self,unit)) or (unit == "focus" and CreateFocusFrame(self,unit)) or (unit == "focustarget" and CreateTargetFocusFrame(self,unit)) or CreatePetFrame(self,unit);
end

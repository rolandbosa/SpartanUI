--[[
	...
	Special Thank's to Hungtar to allow me to use his bag mod.
--]]


local bags_noslots = "can't buy anymore slots!"
local bags_costs = "Cost: %.2f gold"
local bags_buyslots = "Buy new slot with /bags purchase yes"
local bags_openbank = "You need to open your bank first."
local bags_sort = "sort your bags or your bank, if open."
local bags_stack = "fill up partial stacks in your bags or bank, if open."
local bags_buybankslot = "buy bank slot. (need to have bank open)"
local bags_search = "Search"
local bags_sortmenu = "Sort"
local bags_sortspecial = "Sort Special"
local bags_stackmenu = "Stack"
local bags_stackspecial = "Stack Special"
local bags_showbags = "Show Bags"
local bags_sortingbags = "Sorting finished."
local bags_nothingsort= "Nothing to sort."
local bags_bids = "Using bags: "
local bags_stackend = "Restacking finished."
local bags_rightclick_search = "Right-click to search."

local bags_BACKPACK = {0, 1, 2, 3, 4}
local bags_BANK = {-1, 5, 6, 7, 8, 9, 10, 11}
local BAGSFONT = "Interface\\Addons\\Stuffing\\ARIAL.TTF"
local ST_NORMAL = 1
local ST_FISHBAG = 2
local ST_SPECIAL = 3
local bag_bars = 0
...
local function Print (x)
	DEFAULT_CHAT_FRAME:AddMessage("|cffC495DDColdui:|r " .. x)
end
...
function Stuffing:SlotUpdate(b)
	--if not ColdBags:IsShown() then return end -- don't do any slot update if bags are not show
	local texture, count, locked = GetContainerItemInfo (b.bag, b.slot)
	local clink = GetContainerItemLink(b.bag, b.slot)
	
	-- set all slot color to default Coldui on update
	if not b.frame.lock then
		b.frame:SetBackdropBorderColor(.6,.6,.6)
	end
...
function Stuffing:CreateBagFrame(w)
	...
	f.b_close:SetScript("OnClick", function(self, btn)
		if self:GetParent():GetName() == "ColdBags" and btn == "RightButton" then
...
function Stuffing:InitBags()
	...
	local tooltip_show = function (self)
		...
		GameTooltip:SetText(bags_rightclick_search)
	end
...
function Stuffing:Layout(lb)
	...
	if lb then
		...
		f.editbox:SetFont(BAGSFONT, 12, "OUTLINE, MONOCHROME")
		f.detail:SetFont(BAGSFONT, 12, "OUTLINE, MONOCHROME")
		f.gold:SetFont(BAGSFONT, 12, "OUTLINE, MONOCHROME")
	...
	if bag_bars == 1 then
		...
		fb:SetBackdropColor(.2,.2,.2,.6)
		fb:SetBackdropBorderColor(0,0,0)
	...
	f:SetWidth(cols * 31 + (cols - 1) * 4 + 12 * 2)
	f:SetHeight(rows * 31 + (rows - 1) * 4 + off + 12 * 2)
	...
	for _, i in ipairs(bs) do
		...
		if bag_cnt > 0 then
			...
			for j = 1, bag_cnt do
				...
				b.frame:SetBackdropColor(.2,.2,.2,.6)
				b.frame:SetBackdropBorderColor(0,0,0)
				...
				iconTex:SetPoint("TOPLEFT", b.frame, 1, -1)
				iconTex:SetPoint("BOTTOMRIGHT", b.frame, -1, 1)
...
local function StuffingSlashCmd(Cmd)
	...
	elseif cmd == "purchase" then
		...
		if Stuffing.bankFrame and Stuffing.bankFrame:IsShown() then
			...
			Print(string.format(bags_costs, GetBankSlotCost() / 10000))
			Print(bags_buyslots)
		else
			Print(bags_openbank)
		end
	else
		Print("sort - " .. bags_sort)
		Print("stack - " .. bags_stack)
		Print("purchase - " .. bags_buybankslot)
	end
end
...
function Stuffing:ADDON_LOADED(addon)
	...
	tinsert(UISpecialFrames,"ColdBags")
...
function Stuffing:PLAYER_ENTERING_WORLD()
	...
	ContainerFrame1:HookScript("OnShow", function(self)
		...
		for i=1, GetKeyRingSize() do
			...
			t:SetPoint("TOPLEFT", slot, 1, -1)
			t:SetPoint("BOTTOMRIGHT", slot, -1, 1)
		end		
	end)
	
	ContainerFrame1:ClearAllPoints()
	ContainerFrame1:SetPoint("BOTTOMRIGHT", ColdBags, "TOPRIGHT", 0, 5)
	ContainerFrame1.ClearAllPoints = function() end
